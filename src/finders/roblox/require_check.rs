...
pub struct RequireCheckFinder {
    reader: Arc<dyn MemoryReader>,
}

impl RequireCheckFinder {
    ...
    fn find_by_pattern(&self, start: Address, end: Address) -> Option<FinderResult> {
        let patterns = vec![
            Pattern::from_hex("FD 7B ?? A9 FD ?? ?? 91 F3 ?? ?? A9 ?? ?? ?? 90 ?? ?? ?? 91"),
            Pattern::from_hex("90 ?? ?? ?? 91 ?? ?? ?? F9 ?? ?? ?? B4 ?? ?? ?? 94"),
        ];
            ...
            while current < end {
                if let Ok(bytes) = self.reader.read_bytes(current, 4096) {
                    if let Some(offset) = pattern.find_in(&bytes) {
                        ...
                        if self.validate_require_check(addr) {
                            return Some(FinderResult {
                                name: "RequireCheck".to_string(),
                                address: addr,
                                confidence: 0.85,
                                method: "pattern".to_string(),
                                category: "roblox".to_string(),
                                signature: Some("bool RequireCheck(lua_State* L, ModuleScript* script)".to_string()),
                            });
                        }
                    }
                }
        ...
        for needle in &search_strings {
            if let Some(string_addr) = self.find_string(needle, start, end) {
                if let Some(func_addr) = self.find_xref_to_string(string_addr, start, end) {
                    ...
                    if self.validate_require_check(func_start) {
                        return Some(FinderResult {
                            name: "RequireCheck".to_string(),
                            address: func_start,
                            confidence: 0.82,
    ...
    fn find_by_heuristic(&self, start: Address, end: Address) -> Option<FinderResult> {
        ...
        while current < end {
            if let Ok(bytes) = self.reader.read_bytes(current, 128) {
                if self.looks_like_require_check(&bytes) {
    ...
    fn validate_require_check(&self, addr: Address) -> bool {
        if let Ok(bytes) = self.reader.read_bytes(addr, 128) {
            ...
            let mut has_module_check = false;
            let mut has_permission_check = false;
            ...
            return has_module_check && (has_permission_check || has_error_path);
        }

        false
    }

    fn looks_like_require_check(&self, bytes: &[u8]) -> bool {
        if bytes.len() < 32 {
            return false;
        }
        ...
        load_count >= 2 && cmp_count >= 1 && branch_count >= 1
    }
...
pub fn find_require_check(reader: Arc<dyn MemoryReader>, start: Address, end: Address) -> Option<FinderResult> {
    RequireCheckFinder::new(reader).find(start, end)
}
