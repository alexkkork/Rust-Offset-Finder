...
pub struct TaskDeferFinder {
    reader: Arc<dyn MemoryReader>,
}

impl TaskDeferFinder {
    ...
    fn find_by_pattern(&self, start: Address, end: Address) -> Option<FinderResult> {
        let patterns = vec![
            Pattern::from_hex("FD 7B ?? A9 FD ?? ?? 91 F3 ?? ?? A9 F5 ?? ?? A9 ?? ?? ?? 90"),
            Pattern::from_hex("A9 ?? ?? ?? A9 ?? ?? ?? 94 ?? ?? ?? B4 ?? ?? ?? 94"),
        ];
                    ...
                    if let Some(offset) = pattern.find_in(&bytes) {
                        ...
                        if self.validate_task_defer(addr) {
                            return Some(FinderResult {
                                ...
                                signature: Some("int taskDefer(lua_State* L)".to_string()),
                            });
                        }
                    }
                }
    ...
    fn find_by_heuristic(&self, start: Address, end: Address) -> Option<FinderResult> {
        ...
        while current < end {
            if let Ok(bytes) = self.reader.read_bytes(current, 128) {
                if self.looks_like_task_defer(&bytes) {
    ...
    fn validate_task_defer(&self, addr: Address) -> bool {
        if let Ok(bytes) = self.reader.read_bytes(addr, 128) {
            ...
            return has_scheduler_call && (has_thread_create || has_queue_add);
        }

        false
    }

    fn looks_like_task_defer(&self, bytes: &[u8]) -> bool {
        if bytes.len() < 32 {
            return false;
        }
...
pub fn find_task_defer(reader: Arc<dyn MemoryReader>, start: Address, end: Address) -> Option<FinderResult> {
    TaskDeferFinder::new(reader).find(start, end)
}
