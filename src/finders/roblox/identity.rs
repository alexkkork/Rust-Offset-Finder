...
impl IdentityPropagatorFinder {
    ...
    fn find_by_pattern(&self, start: Address, end: Address) -> Option<FinderResult> {
        let patterns = vec![
            Pattern::from_hex("FD 7B ?? A9 FD ?? ?? 91 F3 ?? ?? A9 ?? ?? ?? B9 71"),
            Pattern::from_hex("B9 ?? ?? ?? 71 ?? ?? ?? 54 ?? ?? ?? B9 ?? ?? ?? 91"),
    ...
    fn find_by_heuristic(&self, start: Address, end: Address) -> Option<FinderResult> {
        ...
        while current < end {
            if let Ok(bytes) = self.reader.read_bytes(current, 128) {
                if self.looks_like_identity_propagator(&bytes) {
    ...
    fn validate_identity_propagator(&self, addr: Address) -> bool {
        if let Ok(bytes) = self.reader.read_bytes(addr, 128) {
            ...
            let mut has_extraspace_access = false;
            let mut has_identity_write = false;
            ...
            return has_extraspace_access && (has_identity_write || has_cmp);
        }

        false
    }

    fn looks_like_identity_propagator(&self, bytes: &[u8]) -> bool {
        if bytes.len() < 32 {
            return false;
        }
...
pub fn find_identity_propagator(reader: Arc<dyn MemoryReader>, start: Address, end: Address) -> Option<FinderResult> {
    IdentityPropagatorFinder::new(reader).find(start, end)
}
